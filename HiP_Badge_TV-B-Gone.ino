...
TV-B-Gone Firmware version 1.2
 ...
 13-September-2015
 Now also works with Arduino Leonardo (which is not relevant for the ESP32 version of the code)
 ...
 Updated definitions for:
      extern PGM_P *NApowerCodes[] ;
      extern PGM_P *EUpowerCodes[] ;
 to:
 ...
 19-Apr-2025
 Start porting to ESP32C3 for the HOPE_16 badge
...
#include "main.h"
#include <avr/sleep.h>
#include <Arduino.h>    // needed for ESP32 on the Arduino IDE
...
/* This is kind of a strange but very useful helper function
 ...
 // NOTE: This was important when using an ATtiny85 microcontroller, but no longer important for an ESP32C3.
...
uint8_t read_bits(uint8_t count)
{
  ...
  // we need to read back the number of bytes in the "count" variable
  for (i=0; i<count; i++) {
...
/*
The C compiler for AVR chips (including ATtiny85 and ATmega328P)
 creates code that will transfer all constants into RAM when
 ...
 (this is used in the definition for powerCodes).  
 Since the C compiler assumes that constants are in RAM, and not in program memory, 
 when accessing powerCodes, we need to use the pgm_read_word() and pgm_read_byte macros, 
 and we need to use powerCodes as an address.  This is done with PGM_P, defined
 below.
